"""Autorizacao OAuth 2.0 para Google Drive e OneDrive.
            ...
            def do_GET(self):
                ...
                if "error" in dados:
                    ...
                    corpo = _PAGINA % ("Tudo certo!",
                                       "O FTPZilla ja recebeu a autorizacao.")
                else:
...
def sessao_de(site, nome_provedor: str) -> Sessao:
    """Monta a Sessao a partir dos campos de um sites.Site."""
    return Sessao(
        nome_provedor,
        client_id=str(site.get("client_id", "") or ""),
        refresh_token=str(site.get("refresh_token", "") or ""),
        client_secret=str(site.get("client_secret", "") or ""),
        tenant=str(site.get("tenant", "") or "") or "common",
    )
